(function () {
    $(document).ready(function () {
        var walkthrough1;
        walkthrough1 = {
            index: 0,
            nextScreen: function () {
                if (this.index < this.indexMax()) {
                    this.index++;
                    return this.updateScreen();
                }
            },
            prevScreen: function () {
                if (this.index > 0) {
                    this.index--;
                    return this.updateScreen();
                }
            },
            updateScreen: function () {
                this.reset();
                this.goTo(this.index);
                return this.setBtns();
            },
            setBtns: function () {
                var $lastBtn, $nextBtn, $prevBtn;
                $nextBtn = $('.next-screen1');
                $prevBtn = $('.prev-screen1');
                $lastBtn = $('.finish');
                if (walkthrough1.index === walkthrough1.indexMax()) {
                    $nextBtn.prop('disabled', true);
                    $prevBtn.prop('disabled', false);
                    return $lastBtn.addClass('active').prop('disabled', false);
                } else if (walkthrough1.index === 0) {
                    $nextBtn.prop('disabled', false);
                    $prevBtn.prop('disabled', true);
                    return $lastBtn.removeClass('active').prop('disabled', true);
                } else {
                    $nextBtn.prop('disabled', false);
                    $prevBtn.prop('disabled', false);
                    return $lastBtn.removeClass('active').prop('disabled', true);
                }
            },
            goTo: function (index) {
                $('.screen1').eq(index).addClass('active');
                return $('.dot').eq(index).addClass('active');
            },
            reset: function () {
                return $('.screen1, .dot').removeClass('active');
            },
            indexMax: function () {
                return $('.screen1').length - 1;
            },
            closeModal: function () {
                $('.walkthrough1, .shade').removeClass('reveal');
                return setTimeout((() => {
                    $('.walkthrough1, .shade').removeClass('show');
                    this.index = 0;
                    return this.updateScreen();
                }), 200);
            },
            openModal: function () {
                $('.walkthrough1, .shade').addClass('show');
                setTimeout((() => {
                    return $('.walkthrough1, .shade').addClass('reveal');
                }), 200);
                return this.updateScreen();
            }
        };
        $('.next-screen1').click(function () {
            return walkthrough1.nextScreen();
        });
        $('.prev-screen1').click(function () {
            return walkthrough1.prevScreen();
        });
        $('.close').click(function () {
            return walkthrough1.closeModal();
        });
        $('.open-walkthrough1').click(function () {
            return walkthrough1.openModal();
        });
        walkthrough1.openModal();

        // Optionally use arrow keys to navigate walkthrough1
        return $(document).keydown(function (e) {
            switch (e.which) {
                case 37:
                    // left
                    walkthrough1.prevScreen();
                    break;
                case 38:
                    // up
                    walkthrough1.openModal();
                    break;
                case 39:
                    // right
                    walkthrough1.nextScreen();
                    break;
                case 40:
                    // down
                    walkthrough1.closeModal();
                    break;
                default:
                    return;
            }
            e.preventDefault();
        });
    });

}).call(this);

//# sourceMappingURL=data:application/json;base64,
//# sourceURL=coffeescript
/*----------------------------------------------

section2

---------------------------------------------------- */
(function () {
    $(document).ready(function () {
        var walkthrough2;
        walkthrough2 = {
            index: 0,
            nextScreen: function () {
                if (this.index < this.indexMax()) {
                    this.index++;
                    return this.updateScreen();
                }
            },
            prevScreen: function () {
                if (this.index > 0) {
                    this.index--;
                    return this.updateScreen();
                }
            },
            updateScreen: function () {
                this.reset();
                this.goTo(this.index);
                return this.setBtns();
            },
            setBtns: function () {
                var $lastBtn, $nextBtn, $prevBtn;
                $nextBtn = $('.next-screen2');
                $prevBtn = $('.prev-screen2');
                $lastBtn = $('.finish');
                if (walkthrough2.index === walkthrough2.indexMax()) {
                    $nextBtn.prop('disabled', true);
                    $prevBtn.prop('disabled', false);
                    return $lastBtn.addClass('active').prop('disabled', false);
                } else if (walkthrough2.index === 0) {
                    $nextBtn.prop('disabled', false);
                    $prevBtn.prop('disabled', true);
                    return $lastBtn.removeClass('active').prop('disabled', true);
                } else {
                    $nextBtn.prop('disabled', false);
                    $prevBtn.prop('disabled', false);
                    return $lastBtn.removeClass('active').prop('disabled', true);
                }
            },
            goTo: function (index) {
                $('.screen2').eq(index).addClass('active');
                return $('.dot').eq(index).addClass('active');
            },
            reset: function () {
                return $('.screen2, .dot').removeClass('active');
            },
            indexMax: function () {
                return $('.screen2').length - 1;
            },
            closeModal: function () {
                $('.walkthrough2, .shade').removeClass('reveal');
                return setTimeout((() => {
                    $('.walkthrough2, .shade').removeClass('show');
                    this.index = 0;
                    return this.updateScreen();
                }), 200);
            },
            openModal: function () {
                $('.walkthrough2, .shade').addClass('show');
                setTimeout((() => {
                    return $('.walkthrough2, .shade').addClass('reveal');
                }), 200);
                return this.updateScreen();
            }
        };
        $('.next-screen2').click(function () {
            return walkthrough2.nextScreen();
        });
        $('.prev-screen2').click(function () {
            return walkthrough2.prevScreen();
        });
        $('.close').click(function () {
            return walkthrough2.closeModal();
        });
        $('.open-walkthrough2').click(function () {
            return walkthrough2.openModal();
        });
        walkthrough2.openModal();

        // Optionally use arrow keys to navigate walkthrough2
        return $(document).keydown(function (e) {
            switch (e.which) {
                case 37:
                    // left
                    walkthrough2.prevScreen();
                    break;
                case 38:
                    // up
                    walkthrough2.openModal();
                    break;
                case 39:
                    // right
                    walkthrough2.nextScreen();
                    break;
                case 40:
                    // down
                    walkthrough2.closeModal();
                    break;
                default:
                    return;
            }
            e.preventDefault();
        });
    });

}).call(this);

//# sourceMappingURL=data:application/json;base64,
//# sourceURL=coffeescript
/*----------------------------------------------

section3

---------------------------------------------------- */
(function () {
    $(document).ready(function () {
        var walkthrough3;
        walkthrough3 = {
            index: 0,
            nextScreen: function () {
                if (this.index < this.indexMax()) {
                    this.index++;
                    return this.updateScreen();
                }
            },
            prevScreen: function () {
                if (this.index > 0) {
                    this.index--;
                    return this.updateScreen();
                }
            },
            updateScreen: function () {
                this.reset();
                this.goTo(this.index);
                return this.setBtns();
            },
            setBtns: function () {
                var $lastBtn, $nextBtn, $prevBtn;
                $nextBtn = $('.next-screen3');
                $prevBtn = $('.prev-screen3');
                $lastBtn = $('.finish');
                if (walkthrough3.index === walkthrough3.indexMax()) {
                    $nextBtn.prop('disabled', true);
                    $prevBtn.prop('disabled', false);
                    return $lastBtn.addClass('active').prop('disabled', false);
                } else if (walkthrough3.index === 0) {
                    $nextBtn.prop('disabled', false);
                    $prevBtn.prop('disabled', true);
                    return $lastBtn.removeClass('active').prop('disabled', true);
                } else {
                    $nextBtn.prop('disabled', false);
                    $prevBtn.prop('disabled', false);
                    return $lastBtn.removeClass('active').prop('disabled', true);
                }
            },
            goTo: function (index) {
                $('.screen3').eq(index).addClass('active');
                return $('.dot').eq(index).addClass('active');
            },
            reset: function () {
                return $('.screen3, .dot').removeClass('active');
            },
            indexMax: function () {
                return $('.screen3').length - 1;
            },
            closeModal: function () {
                $('.walkthrough3, .shade').removeClass('reveal');
                return setTimeout((() => {
                    $('.walkthrough3, .shade').removeClass('show');
                    this.index = 0;
                    return this.updateScreen();
                }), 200);
            },
            openModal: function () {
                $('.walkthrough3, .shade').addClass('show');
                setTimeout((() => {
                    return $('.walkthrough3, .shade').addClass('reveal');
                }), 200);
                return this.updateScreen();
            }
        };
        $('.next-screen3').click(function () {
            return walkthrough3.nextScreen();
        });
        $('.prev-screen3').click(function () {
            return walkthrough3.prevScreen();
        });
        $('.close').click(function () {
            return walkthrough3.closeModal();
        });
        $('.open-walkthrough3').click(function () {
            return walkthrough3.openModal();
        });
        walkthrough3.openModal();

        // Optionally use arrow keys to navigate walkthrough3
        return $(document).keydown(function (e) {
            switch (e.which) {
                case 37:
                    // left
                    walkthrough3.prevScreen();
                    break;
                case 38:
                    // up
                    walkthrough3.openModal();
                    break;
                case 39:
                    // right
                    walkthrough3.nextScreen();
                    break;
                case 40:
                    // down
                    walkthrough3.closeModal();
                    break;
                default:
                    return;
            }
            e.preventDefault();
        });
    });

}).call(this);

//# sourceMappingURL=data:application/json;base64,
//# sourceURL=coffeescript
/*----------------------------------------------

section4

---------------------------------------------------- */
(function () {
    $(document).ready(function () {
        var walkthrough4;
        walkthrough4 = {
            index: 0,
            nextScreen: function () {
                if (this.index < this.indexMax()) {
                    this.index++;
                    return this.updateScreen();
                }
            },
            prevScreen: function () {
                if (this.index > 0) {
                    this.index--;
                    return this.updateScreen();
                }
            },
            updateScreen: function () {
                this.reset();
                this.goTo(this.index);
                return this.setBtns();
            },
            setBtns: function () {
                var $lastBtn, $nextBtn, $prevBtn;
                $nextBtn = $('.next-screen4');
                $prevBtn = $('.prev-screen4');
                $lastBtn = $('.finish');
                if (walkthrough4.index === walkthrough4.indexMax()) {
                    $nextBtn.prop('disabled', true);
                    $prevBtn.prop('disabled', false);
                    return $lastBtn.addClass('active').prop('disabled', false);
                } else if (walkthrough4.index === 0) {
                    $nextBtn.prop('disabled', false);
                    $prevBtn.prop('disabled', true);
                    return $lastBtn.removeClass('active').prop('disabled', true);
                } else {
                    $nextBtn.prop('disabled', false);
                    $prevBtn.prop('disabled', false);
                    return $lastBtn.removeClass('active').prop('disabled', true);
                }
            },
            goTo: function (index) {
                $('.screen4').eq(index).addClass('active');
                return $('.dot').eq(index).addClass('active');
            },
            reset: function () {
                return $('.screen4, .dot').removeClass('active');
            },
            indexMax: function () {
                return $('.screen4').length - 1;
            },
            closeModal: function () {
                $('.walkthrough4, .shade').removeClass('reveal');
                return setTimeout((() => {
                    $('.walkthrough4, .shade').removeClass('show');
                    this.index = 0;
                    return this.updateScreen();
                }), 200);
            },
            openModal: function () {
                $('.walkthrough4, .shade').addClass('show');
                setTimeout((() => {
                    return $('.walkthrough4, .shade').addClass('reveal');
                }), 200);
                return this.updateScreen();
            }
        };
        $('.next-screen4').click(function () {
            return walkthrough4.nextScreen();
        });
        $('.prev-screen4').click(function () {
            return walkthrough4.prevScreen();
        });
        $('.close').click(function () {
            return walkthrough4.closeModal();
        });
        $('.open-walkthrough4').click(function () {
            return walkthrough4.openModal();
        });
        walkthrough4.openModal();

        // Optionally use arrow keys to navigate walkthrough4
        return $(document).keydown(function (e) {
            switch (e.which) {
                case 37:
                    // left
                    walkthrough4.prevScreen();
                    break;
                case 38:
                    // up
                    walkthrough4.openModal();
                    break;
                case 39:
                    // right
                    walkthrough4.nextScreen();
                    break;
                case 40:
                    // down
                    walkthrough4.closeModal();
                    break;
                default:
                    return;
            }
            e.preventDefault();
        });
    });

}).call(this);


//# sourceMappingURL=data:application/json;base64,
//# sourceURL=coffeescript
